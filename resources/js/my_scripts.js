...
function loadStatsPage(){
	...
	for(rowIndex = 2; rowIndex < table.rows.length; rowIndex++){
		...
		if(parseInt(table.rows[rowIndex].cells[2].innerHTML) > parseInt(table.rows[rowIndex].cells[3].innerHTML)){
			table.rows[rowIndex].cells[4].innerHTML = "CU Boulder"
			wins++;
		}

		else{
			table.rows[rowIndex].cells[4].innerHTML = table.rows[rowIndex].cells[1].innerHTML
			losses++;
		}
	}
